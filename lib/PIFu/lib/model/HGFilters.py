...
class HourGlass(nn.Module):
    ...
    def _forward(self, level, inp):
        ...
        up2 = F.interpolate(low3, scale_factor=2, mode='bicubic', align_corners=False)
...
class HGFilter(nn.Module):
    def __init__(self, opt):
        ...
        if self.opt.hg_down == 'conv64':
            ...
            self.down_conv2 = nn.Conv2d(64, 128, kernel_size=4, stride=2, padding=1)
        elif self.opt.hg_down == 'conv128':
            self.conv2 = ConvBlock(64, 128, self.opt.norm)
            self.down_conv2 = nn.Conv2d(128, 128, kernel_size=4, stride=2, padding=1)
