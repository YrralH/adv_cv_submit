...
def computePRT(mesh, n, order):
    mesh = trimesh.Trimesh(vertices=mesh["v"]*100, faces=mesh["f"])
...
def testPRT(dir_path, n=40):
    mesh = np.load(os.path.join(dir_path,"mesh.npz"))
    PRT, F = computePRT(mesh, n, 2)
    np.savetxt(os.path.join(dir_path, 'bounce0.txt'), PRT, fmt='%.8f')
    np.save(os.path.join(dir_path, 'face.npy'), F)

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('-b', '--base', type=str, default='/mnt/data/fq/TMP/vrc/raw/')
    parser.add_argument('-s', '--src',  type=int, default=0)
    parser.add_argument('-t', '--tar',  type=int, default=344)
    args = parser.parse_args()

    base = args.base
    dlist = sorted(os.listdir(base))
    lll = len(dlist)
    nnn = int(np.ceil(lll/args.src))
    fro = nnn*args.tar
    tto = nnn*(args.tar+1)
    if tto > lll:
        tto = lll
